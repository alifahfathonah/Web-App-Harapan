<?php 
	...
	//	$note_sekolah= mysqli_real_escape_string($db, $_POST['note_sekolah']);
		$nosttb = mysqli_real_escape_string($db, $_POST['nosttb']);
		$date = date('Y-m-d');
		if ($naleng !="") {

			$sql = mysqli_query($db, "INSERT INTO siswa (id_sekolah, nosttb, nama, tempat_lahir1, tgl1, jk, gol_darah1, alamat1, anak_no, kandung, tiri, angkat, anak_tsb, bahasa, warga1, agama1, notel1, email, n_ayah, n_ayah_t, tempat_lahir2, tgl2, agama2, p_ayah, warga2, alamat2, notel2, alamat3, notel3, gol_darah2, n_ibu, n_ibu_t, tempat_lahir3, tgl3, agama3, p_ibu, warga3, alamat4, notel4, alamat5, notel5, gol_darah3, tgl_daftar,id_asal,note_sekolah) VALUES ('$id', '$nosttb', '$naleng', '$tempat_lahir1', '$tgl1', '$jk', '$gol_darah1', '$alamat1', '$anak_no', '$kandung', '$tiri', '$angkat', '$anak_tsb', '$bahasa', '$warga1', '$agama1', '$notel1', '$email', '$n_ayah', '$n_ayah_t', '$tempat_lahir2', '$tgl2', '$agama2', '$p_ayah', '$warga2', '$alamat2', '$notel2', '$alamat3', '$notel3', '$gol_darah2', '$n_ibu', '$n_ibu_t', '$tempat_lahir3', '$tgl3', '$agama3', '$p_ibu', '$warga3', '$alamat4', '$notel4', '$alamat5', '$notel5', '$gol_darah3', '$date','11','$note_sekolah')");
			$jk=mysqli_insert_id($db);
			
			echo "
				<script>
					window.location.href = '?page=umum/harga.php&id=$jk'
				</script>
			";	
		} else {
			echo "
				<script>
					...
					window.location.href = '?page=umum/form.php'
				</script>
			";	
		}
